...
3. Parches según tu caso: `patch_001` … `patch_004_accounts_reports.sql` (datos de cuenta, comisiones). `patch_005_account_kind.sql` clasifica **banco / wallet / app** (Zinly aparte de Binance y del banco). `patch_007_transaction_counterpart.sql` habilita **traspasos entre cuentas** y la columna **cuenta relacionada** en Movimientos. `patch_009_goals_budgets.sql` agrega **metas de ahorro**, **presupuestos por rubro** y **fondo de emergencia** en el Dashboard.
